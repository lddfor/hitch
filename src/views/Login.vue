<template>
  <div class="login-view">
    <el-card>
      <div slot="header">{{ $t('login.title') }}</div>
      <el-form
...
export default {
  ...
  methods: {
    ...
    login() {
      ...
      this.$router.push(this.$route.query.redirect || '/survey/organization/1').catch(err=>{});

      // this.$axios
      //   .post('/auth', this.user)
      //   .then(() => {
      //     this.USER_LOGIN({ user: this.user, remember: this.remember });
      //     this.$router.push(this.$route.query.redirect || '/survey/organization/1');
      //   })
      //   .catch(() => {
